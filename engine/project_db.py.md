...
| `DDL_SYSTEM_TABLES` | SQL to create `_tools`, `_columns`, `_trash`, `_overrides`, `_audit`, `_flags`, `_cell_flags`, `_etl_versions`, `_templates` |
| `create_project_db(path)` | Creates a new per-project DB with the full schema at `SCHEMA_VERSION` |
...
| `audit(conn, tool_slug, action, row_tag, col_slug, old_val, new_val, revision, change_type)` | Inserts a row into `_audit` |
