...
| 59–83 | `onKeydown(e, input)` | Called first by `CellKeyboard.onCellKeydown`; intercepts ArrowDown, ArrowUp, Alt/Tab (accept), Escape; returns `true` to consume the event |
...
- **Auto-highlight first suggestion** (`_selectedIdx = 0`): Alt or Tab key immediately accepts the top suggestion. Tab accepts without moving to the next cell (event is consumed).
