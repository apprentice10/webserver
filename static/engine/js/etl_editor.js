const EtlEditor = (() => {
    ...
    function setToolType(type) { _toolType = type || null; EtlPersistence.configure(_toolType); }

    async function init() {
        if (!_toolType) {
            try { const t = await ApiClient.loadTool(); _toolType = t.tool_type || null; } catch (_) {}
        }
        EtlPersistence.configure(_toolType);
        document.addEventListener("etl:loadModel", e => { _model = e.detail.model; _renderModel(); });
        try {
    ...
    // --------------------------------------------------------
    // TEMPLATES + FILE I/O  (delegated to EtlPersistence)
    // --------------------------------------------------------

    async function refreshTemplates() { return EtlPersistence.refreshTemplates(); }
    async function saveAsTemplate()   { return EtlPersistence.saveAsTemplate(_model); }
    async function loadTemplate(id)   { return EtlPersistence.loadTemplate(id, JSON.stringify(_model)); }
    async function deleteTemplate(id) { return EtlPersistence.deleteTemplate(id); }
    function importFromFile()         { EtlPersistence.importFromFile(); }
    function exportToFile()           { EtlPersistence.exportToFile(_model); }
    function importFromSql()          { EtlPersistence.importFromSql(); }
